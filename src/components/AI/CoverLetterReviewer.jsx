...
export default function CoverLetterReviewer() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Cover Letter Reviewer</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Analyse tone, relevance, gaps, and get rewrite suggestions.</p>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Job Description</label>
          <textarea
            value={jobDescription}
            onChange={e => setJobDescription(e.target.value)}
            placeholder="Paste the full job description here..."
            rows={5}
            className="w-full p-3 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-600 bg-slate-50 dark:bg-slate-800 focus:outline-none focus:border-sky-400 focus:bg-white dark:focus:bg-slate-800 resize-none"
          />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Your Cover Letter</label>
          <textarea
            value={coverLetter}
    ...
    <div className="space-y-4">
      ...
      {result.missing?.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-5">
          <p className="font-bold text-slate-900 dark:text-slate-100 text-sm mb-3">What's Missing</p>
          ...
          {result.rewrites.map((r, i) => (
            <div key={i} className="space-y-2">
              <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800/50 rounded-lg p-3">
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300 italic">"{r.original}"</p>
              </div>
              <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 rounded-lg p-3">
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300">"{r.rewrite}"</p>
              </div>
              {r.reason && <p className="text-xs text-slate-500 dark:text-slate-500 px-1">{r.reason}</p>}
            </div>
          ))}
        </div>
      )}
